...
#Edit Later
CATEGORY_MAP = {
    "Business Trip": "catering.restaurant,service.financial,commercial.convenience",
    "Family Adventure": "leisure.playground,entertainment.amusement_park,tourism.attraction,education.library",
    "Cultural Immersion": "tourism.sights,entertainment.culture.museum,tourism.history,tourism.heritage",
    "Outdoor/Nature Escape": "leisure.park,natural.water,natural.mountain,natural.valley",
    "Nightlife/Entertainment": "catering.bar,catering.pub,entertainment.night_club,entertainment.culture.cinema"
}

def SearchPOIs(location_name: str, user_category: str, lon: float, lat: float):
    categories = CATEGORY_MAP.get(user_category, "tourism.sights")
    ...
    try:
        ...
        if features:
            ...
            print(f"No points of interest found for '{user_category}' in {location_name}.")

    except requests.exceptions.RequestException as e:
        print(f"Geoapify API Error: {e}")

SearchPOIs("Kyoto", "Business Trip", 135.7681, 35.0116)


#LATER FIX THIS FOR FINDING HOTELS
def FindHotel(lon: float, lat: float, check_in_date: str, check_out_date: str):
    try:
        amadeus = Client(
